...
const getMathQuestion = async (req, res) => {
    ...
    try {
        const user = await User.findById(userId).select('mathsQuestionElo level exp coins hp extracoins name lastHpLoss hpLimit');
        ...
        const level = getLevelByElo(user.mathsQuestionElo);
        taskProgress(user._id, 'task_level', { taskLevel: level });

        const mathQuestionsDoc = await MathsQuestion.findOne({ level });
        if (!mathQuestionsDoc || mathQuestionsDoc.questions.length === 0) {
            return res.status(404).json({ message: 'Brak dostępnych pytań dla tego poziomu.' });
        }

        const randomIndex = Math.floor(Math.random() * mathQuestionsDoc.questions.length);
        const randomQuestion = mathQuestionsDoc.questions[randomIndex];
        ...
        res.status(200).json({
            ...
            userStats: {
                ...
                currentElo: user.mathsQuestionElo,
                currentHp: user.hp,
...
const submitMathAnswer = async (req, res) => {
    const { userId } = req.user;
    const { questionId, answer } = req.body;
    try {
        const user = await User.findById(userId).select('mathsQuestionElo coins exp level hp name extracoins correct_mathsQuestion lastHpLoss hpLimit');
        if (!user) return res.status(404).json({ message: 'Użytkownik nie znaleziony.' });
        ...
        const level = getLevelByElo(user.mathsQuestionElo);
        const questionDoc = await MathsQuestion.findOne(
            { level, 'questions.id_question': questionId },
            { 'questions.$': 1 }
        );
        if (!questionDoc) return res.status(404).json({ message: 'Pytanie nie znalezione.' });
        ...
        user.mathsQuestionElo += isCorrect ? 100 : -100;
        user.mathsQuestionElo = Math.max(0, user.mathsQuestionElo);
        ...
        taskProgress(user._id, 'answered_question', {
            questionDomain: "Math",
            correct_mathsQuestion: user.correct_mathsQuestion,
            questionId,
            isCorrect,
            newElo: user.mathsQuestionElo,
            newCoins: user.coins,
            newExp: user.exp,
            newLevel: user.level,
            hp: user.hp,
        ...
        if (isCorrect) {
            ...
            taskProgress(user._id, 'new_difficulty', { newDifficulty: getLevelByElo(user.mathsQuestionElo) });
        }
...
module.exports = { getMathQuestion, submitMathAnswer };
