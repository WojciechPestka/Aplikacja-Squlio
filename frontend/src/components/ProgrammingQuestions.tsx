...
import './ProgrammingQuestions.css';
...
const ProgrammingQuestion: React.FC<ProgrammingQuestionProps> = ({ level }) => {
  ...
  const fetchQuestion = async () => {
    ...
    try {
      ...
      console.log(`Fetching Programming question for level: ${level}`);
      const response = await fetch(`https://squlio.edu.pl/api/programing-questions/${level}`, {
        headers: {
  ...
  const submitAnswer = async (answer: string, event: React.MouseEvent) => {
    ...
    try {
      ...
      const response = await fetch('https://squlio.edu.pl/api/submit-programing-answer', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className={`programming-question-container level-${userStats?.taskLevel}`}>
      <h1>Programowanie</h1>
        ...
        <div className="question-box">
          ...
          <div className="answers">
            {answers.map((answer) => (
              <button
                key={answer}
                onClick={(event) => submitAnswer(answer, event)} 
