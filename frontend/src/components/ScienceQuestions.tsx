...
const ScienceQuestion: React.FC<ScienceQuestionProps> = ({ level }) => {
  ...
  const fetchQuestion = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://squlio.edu.pl/api/science-questions/${level}`, {
  ...
  const submitAnswer = async (answer: string, event: React.MouseEvent) => {
    ...
    try {
      ...
      const response = await fetch('https://squlio.edu.pl/api/submit-science-answer', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className={`science-question-container level-${userStats?.taskLevel}`}>
      <h1>Przyroda</h1>
